export default class BoundingBox {
    ...
    calculateIntercept(from, to) {
        let tmin = (this.minX - from.x) / (to.x - from.x);
        let tmax = (this.maxX - from.x) / (to.x - from.x);

        if (tmin > tmax) [tmin, tmax] = [tmax, tmin];

        let tymin = (this.minY - from.y) / (to.y - from.y);
        let tymax = (this.maxY - from.y) / (to.y - from.y);
        ...
        let tzmin = (this.minZ - from.z) / (to.z - from.z);
        let tzmax = (this.maxZ - from.z) / (to.z - from.z);
        ...
        // If the entry time is after the exit time, the ray misses the box
        if (tmin > tmax) return null;

        if (tmin >= 0.0 && tmin <= 1.0) return tmin;
        
        // If start is inside the box (tmin < 0) and exit is forward (tmax > 0), return 0
        if (tmin < 0.0 && tmax > 0.0) return 0.0;
