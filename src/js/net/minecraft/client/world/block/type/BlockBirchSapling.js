...
import BirchTreeGenerator from "../../generator/structure/BirchTreeGenerator.js";

export default class BlockBirchSapling extends BlockSapling {
    constructor(id) {
        super(id, 2);
    }
    ...
    onBlockActivated(world, x, y, z, player) {
        ...
        if (item === BlockRegistry.BONE_MEAL.getId()) {
            ...
            if (Math.random() < 0.25) { // Birch grows faster/easier than oak
                this.growTree(world, x, y, z);
            } else {
                world.minecraft.soundManager.playSound("step.grass", x+0.5, y+0.5, z+0.5, 1.0, 1.5);
            }
            return true;
        }
        return false;
    ...
    growTree(world, x, y, z) {
        ...
        let generator = new BirchTreeGenerator(world, world.seed);
    ...
    canPlaceBlockAt(world, x, y, z) {
        ...
        return below === BlockRegistry.GRASS.getId() || below === BlockRegistry.DIRT.getId() || (BlockRegistry.SNOWY_GRASS && below === BlockRegistry.SNOWY_GRASS.getId());
    }
}
